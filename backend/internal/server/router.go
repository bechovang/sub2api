...
func registerRoutes(
	...
	// OptionalJWT and user JWT were only consumed by the stripped panel / auth
	// routes; they remain in the injected signature for wire compatibility.
	_, _ = optionalJWTAuth, jwtAuth

	// 注册各模块路由
	// Strip (Key-Auth Gateway + Admin API only): the built-in panel (model
	// plaza / page routes), the user auth/panel, and all
	// payment/purchase/transaction endpoints are removed. Credit/subscription
	// is granted administratively (Admin API) or by an external FE calling
	// the admin surface with x-api-key (`x-api-key` adminkey). The `jwtAuth` /
	// user session surface is closed.
