...
func RegisterAdminRoutes(
	...
	admin.Use(middleware.AdminComplianceGuard(settingService))
	{
		...
		// Strip: card-redemption (卡密) and promo-code (优惠码) admin surfaces are
		// removed — they only feed the removed purchase/fulfillment flow.
...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	users := admin.Group("/users")
	{
		...
		users.POST("/:id/api-keys", h.Admin.UserAPIKey.CreateUserAPIKey)
		users.PUT("/:id/api-keys/:keyId", h.Admin.UserAPIKey.UpdateUserAPIKey)
		users.DELETE("/:id/api-keys/:keyId", h.Admin.UserAPIKey.DeleteUserAPIKey)
