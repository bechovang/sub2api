...
// ProvidePaymentOrderExpiryService creates PaymentOrderExpiryService.
//
// Strip: the background payment-order reconcile/expiry job is NOT started.
// Payment/purchase is removed, so no order ever needs reconciliation/expiry.
// The service instance is still constructed (to keep the DI cleanup graph wired),
// but it stays inert.
